...
confContent += `addnode=128.199.96.201\n`;
    ...
    this.loadBitcoinloveConf(false);
  };
    ...
    try {
      ...
      if (err === NO_CONNECTION && bitcoinlovedSpawned && getinfoRetryCount >= 10) {
        ...
        this.setState({
          currentStatus: (
            <span>
              Failed to start bitcoinloved. Giving up! Please look at the debug.log file.
              <br />
              <span className={cstyles.highlight}>{`${locateBitcoinloveConfDir()}/debug.log`}</span>
              <br />
          ...
          {creatingBitcoinloveConf && (
            <div>
              <div className={cstyles.verticalflex}>
                ...
                >
                  <div className={[cstyles.xlarge].join(' ')}> Welcome To Blvwallet Fullnode!</div>
                </div>
                ...
                >
                  <div>
                    Blvwallet Fullnode will download the{' '}
                    <span className={cstyles.highlight}>entire Bitcoinlove Blockchain (~2GB)</span>, which might take several
                    hours  to sync. If you want to get started immediately, please consider{' '}
                    <a
                      className={cstyles.highlight}
                      style={{ textDecoration: 'underline' }}
                      role="link"
                      onClick={() => shell.openExternal('https://www.blvwallet.co')}
                    >
                      Blvwallet Lite
                    </a>
